...
import com.epam.serializers.factory.GsonWrapper;
...
public class GsonWpapperTest {
    ...
    public void testWriteObjectToJsonSuccess() throws IOException {
        ...
        gsonWrapper.writeObjectToJson(new File(this.getClass().getResource("/actual/test_book_gson.json").getPath()), testBook, false);

        String actual = "";
        try(BufferedReader reader = new BufferedReader(new FileReader(this.getClass().getResource("/actual/test_book_gson.json").getPath()))){
            while (reader.ready()){
    ...
    public void testReadObjectFromJsonFileSuccess() throws IOException {
        ...
        Book actual = gsonWrapper.readObjectFromJsonFile(new File(this.getClass().getResource("/expected/test_book.json").getPath()), Book.class);
        assertEquals(expected, actual);
    }
    ...
    public void testWriteListObjectsToJsonSuccess() throws IOException {
        ...
        gsonWrapper.writeObjectToJson(new File(this.getClass().getResource("/actual/test_list_book_gson.json").getPath()), books, false);

        String actual = "";
        try(BufferedReader reader = new BufferedReader(new FileReader(this.getClass().getResource("/actual/test_list_book_gson.json").getPath()))){
            while (reader.ready()){
                actual += reader.readLine();
            }
        }
    ...
    public void testReadListElementsFromJsonFile() throws IOException {
        ...
        List<Book> actualBooks = gsonWrapper.readListElementsFromJsonFile(new File(this.getClass().getResource("/expected/list_test_books.json").getPath()), Book.class);
